...
/**
 * POJO used for a settlement batch search results {@code /batches}
 * @see SettlementBatch
 */
@Getter
@Setter
public class BatchSearchResults extends ABaseJSONObject {
	public static final long serialVersionUID = 1L;

	private List<SettlementBatch> items;
	 ...
	 */
	public BatchSearchResults(JSONObject jsonObject) {
		super(jsonObject);
		...
		if (jsonObject.has(JSONMapping.ITEMS)) {
			...
			for (int index = 0;index < itemsArr.length();index++) {
				...
				this.getItems().add(new SettlementBatch(objAtIndex));
			}
		}
	}
