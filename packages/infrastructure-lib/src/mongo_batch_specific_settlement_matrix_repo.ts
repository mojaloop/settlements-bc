...
import {IBatchSpecificSettlementMatrix, ISettlementMatrix} from "@mojaloop/settlements-bc-public-types-lib";

export class MongoBatchSpecificSettlementMatrixRepo implements IBatchSpecificSettlementMatrixRequestRepo {
	...
	async init(): Promise<void> {
		try {
			...
			this._logger.info("MongoBatchSpecificSettlementMatrixRepo - initialised");
		} catch (error: unknown) {
			this._logger.error(error, "MongoBatchSpecificSettlementMatrixRepo - initialisation failed");
	...
	async storeMatrix(req: IBatchSpecificSettlementMatrix): Promise<void>{
		await this._collection.updateOne({id: req.id}, {$set: req}, {upsert: true});
	}

	async getMatrixById(id: string): Promise<IBatchSpecificSettlementMatrix | null>{
		try {
			const batch = await this._collection.findOne({id: id}, {projection: {_id: 0}});
			return batch as (IBatchSpecificSettlementMatrix | null);
		} catch (error: any) {
			throw new Error("Unable to get batch specific matrix from repo - msg: " + error.message);
		}
	}
}
